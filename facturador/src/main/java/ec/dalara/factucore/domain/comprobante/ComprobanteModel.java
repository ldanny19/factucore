...
public final class ComprobanteModel {
	...
	public ComprobanteModel(Long id, Long empresaId, Long establecimientoId, Long puntoEmisionId,
			String codigoDocumento, NumeroComprobante numeroComprobante, LocalDate fechaEmision) {

		if (empresaId == null) {
			throw new DomainException("FACTUCORE.COMPROBANTE.EMPRESA.REQUERIDA");
		}

		if (establecimientoId == null) {
			throw new DomainException("FACTUCORE.COMPROBANTE.ESTABLECIMIENTO.REQUERIDO");
		}

		if (puntoEmisionId == null) {
			throw new DomainException("FACTUCORE.COMPROBANTE.PUNTO_EMISION.REQUERIDO");
		}

		if (codigoDocumento == null || codigoDocumento.isBlank()) {
			throw new DomainException("FACTUCORE.COMPROBANTE.CODIGO_DOCUMENTO.REQUERIDO");
		}

		if (numeroComprobante == null) {
			throw new DomainException("FACTUCORE.COMPROBANTE.NUMERO.REQUERIDO");
		}

		if (fechaEmision == null) {
			throw new DomainException("FACTUCORE.COMPROBANTE.FECHA_EMISION.REQUERIDA");
		}
	...
	public void asignarClaveAcceso(ClaveAcceso claveAcceso) {
		if (claveAcceso == null) {
			throw new DomainException("FACTUCORE.CLAVE_ACCESO.REQUERIDA");
		}
	...
	public void cambiarEstadoProceso(String estadoProceso) {
		if (estadoProceso == null || estadoProceso.isBlank()) {
			throw new DomainException("FACTUCORE.COMPROBANTE.ESTADO_PROCESO.REQUERIDO");
		}
