...
public class SriSoapAdapter implements SriPort {
    ...
    private static final String SOAP_ENV_NAMESPACE =
            "http://schemas.xmlsoap.org/soap/envelope/";

    private static final String RECEPCION_NAMESPACE =
            "http://ec.gob.sri.ws.recepcion";

    private static final String AUTORIZACION_NAMESPACE =
    ...
    public SriResponse recibir(String xml) {
        ...
        if (xml == null || xml.isBlank()) {
            throw new ApplicationException(
                    MessageCodes.SRI_XML_REQUERIDO
            );
        }
        ...
        String respuesta = ejecutarSolicitud(
                url,
                soapRequest
        );
    ...
    public SriResponse autorizar(String claveAcceso) {
        ...
        if (claveAcceso == null || claveAcceso.isBlank()) {
            throw new ApplicationException(
                    MessageCodes.SRI_CLAVE_ACCESO_REQUERIDA
            );
        }

        String url = obtenerUrlAutorizacion();

        String soapRequest = construirSolicitudAutorizacion(
                claveAcceso
        );
    ...
    private String obtenerUrlRecepcion() {
        ...
        SriProperties.AmbienteProperties ambiente =
                obtenerAmbiente();

        if (ambiente.getRecepcionUrl() == null
                || ambiente.getRecepcionUrl().isBlank()) {

            throw new ApplicationException(
                    MessageCodes.SRI_URL_RECEPCION_NO_CONFIGURADA,
    ...
    private String obtenerUrlAutorizacion() {
        ...
        if (ambiente.getAutorizacionUrl() == null
                || ambiente.getAutorizacionUrl().isBlank()) {

            throw new ApplicationException(
                    MessageCodes.SRI_URL_AUTORIZACION_NO_CONFIGURADA,
                    properties.getAmbiente()
            );
        }
    ...
    private SriProperties.AmbienteProperties obtenerAmbiente() {
        ...
        if (properties.getAmbiente() == null
                || properties.getAmbiente().isBlank()) {

            throw new ApplicationException(
                    MessageCodes.SRI_AMBIENTE_REQUERIDO
            );
        }
    ...
    private String construirSolicitudAutorizacion(
            String claveAcceso) {
    ...
    private String ejecutarSolicitud(
            String url,
            String soapRequest) {
    ...
    private SriResponse procesarRespuestaRecepcion(
            String xml) {
        ...
        return new SriResponse(
                "RECIBIDA".equalsIgnoreCase(estado),
                estado,
                identificador,
                mensajes
        );
    ...
    private SriResponse procesarRespuestaAutorizacion(
            String xml) {
    ...
    private List<SriMensaje> obtenerMensajes(
            Element contenedor,
            String elementoContenedor) {

        List<SriMensaje> mensajes =
                new ArrayList<>();

        NodeList nodos =
                contenedor.getElementsByTagNameNS(
                        "*",
                        "mensaje"
                );
    ...
    private Element obtenerPrimerElemento(
            Node nodo,
            String nombre) {

        if (nodo instanceof Element elemento
                && nombre.equals(elemento.getLocalName())) {
        ...
        if (nodo instanceof Document document) {
            ...
            nodos = document.getElementsByTagNameNS(
                    "*",
                    nombre
            );
            ...
            nodos = elemento.getElementsByTagNameNS(
                    "*",
                    nombre
            );
