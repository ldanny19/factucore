...
public class CatalogoItemService extends BaseService<CatalogoItem> {
	...
	public List<CatalogoItem> listarPorCatalogo(Long catalogoId) {
		return catalogoItemRepository.findByCatalogoId(catalogoId).stream()
				.filter(item -> !EstadoRegistro.ELIMINADO.equals(item.getEstadoRegistro())).toList();
	}

	public Optional<CatalogoItem> obtenerPorCatalogoYCodigo(Long catalogoId, String codigo) {
		return catalogoItemRepository.findByCatalogoIdAndCodigo(catalogoId, codigo)
				.filter(item -> EstadoRegistro.ACTIVO.equals(item.getEstadoRegistro()));
	}

	public boolean existePorCatalogoYCodigo(Long catalogoId, String codigo) {
		return catalogoItemRepository.findByCatalogoIdAndCodigo(catalogoId, codigo)
				.filter(item -> !EstadoRegistro.ELIMINADO.equals(item.getEstadoRegistro())).isPresent();
	}
}